    ...
    public void remove(TextView view){
	   ...
	   public boolean loadInfo(TextView view, String path)
	   {
		   ...
		   if(path.equals("to_up_dir")){
			   ...
			   {
				   loaded = loadCachedInfo(view, path, cate);
			   }
		   }
	        if (loaded) {
	            mPendingRequests.remove(view);
	    ...
	    private boolean loadCachedInfo(TextView view, String path, int cate) {
	    ...
	    private void processLoadedInfos() {
	        ...
	        while (iterator.hasNext()) {
	            ...
	            boolean loaded = loadCachedInfo(view, fileId.mPath, fileId.mCategory);
	            if (loaded) {
	                iterator.remove();
	            ...
	            while (iterator.hasNext()) {
	                ...
	                if (holder != null && holder.state == TextHolder.NEEDED) {
	                    ...
	                    switch (id.mCategory) {
	                    	...
	                    	{
	                    		int count = getDirectoryChildrenCount(new File(id.mPath));
	                    		String date = FileUtils.getFileDate(new File(id.mPath));
	                    		if(count==0)
	                    		{
	                        ...
	                        {
	                        	String info = getAudioInfoFromFile(id.mPath);
	                        	String size = FileUtils.getFileSize(new File(id.mPath));
	                        	String date = FileUtils.getFileDate(new File(id.mPath));
