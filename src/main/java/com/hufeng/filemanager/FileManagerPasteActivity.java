 ...
 */
public class FileManagerPasteActivity extends FileOperationActivity implements FileBrowserFragment.FileBrowserCallbacks{

    private FileBrowserFragment mFileBrowserFragment;

    @Override
    protected FileOperation getFileOperation() {
        FileOperation operation =  mFileBrowserFragment.getFileOperation();
        if (operation == null) {
    ...
    protected void onCreate(Bundle arg0) {
        ...
        showFilePasteFragment();

        Intent intent = getIntent();
        if( FileManagerTabActivity.ACTION_MOVE_FILES.equals(intent.getAction()) ) {
            String[] files = intent.getStringArrayExtra("files");
            getFileOperation().setMoveFiles(files);
        } else if( FileManagerTabActivity.ACTION_COPY_FILES.equals(intent.getAction()) ) {
            String[] files = intent.getStringArrayExtra("files");
...
//        }
    ...
    private void showFilePasteFragment() {
        ...
        if(fragment == null) {
            fragment = FileBrowserFragment.newStorageBrowser(null);
