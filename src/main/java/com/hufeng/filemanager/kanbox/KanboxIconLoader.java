 ...
 */
public class KanboxIconLoader implements Handler.Callback{

    private static final String LOG_TAG = KanboxIconLoader.class.getSimpleName();

    private static final String LOADER_THREAD_NAME = "KanboxIconLoader";
    ...
    private static class BitmapHolder {
        ...
        public boolean setBitmapForImageView(ImageView v) {
            ...
            if(v.getTag()!=null) {
                RelativeLayout background = (RelativeLayout)v.getTag();
    ...
    private final String[] COLUMNS = new String[] { DataStructures.CloudBoxColumns.FILE_PATH_FIELD,
    ...
    private static KanboxIconLoader instance = null;

    public static KanboxIconLoader getInstance()
    {
        if(instance==null)
            instance = new KanboxIconLoader(FileManager.getAppContext());
        return instance;
    }
     ...
     */
    private KanboxIconLoader(Context context) {
        mContext = context;
    ...
    public boolean loadIcon(ImageView view, RelativeLayout background, String path)
    {
        boolean loaded;
        if(background!=null) {
        ...
        view.setImageResource(IconLoaderHelper.getFileIcon(type));
        if(FileUtils.FILE_TYPE_IMAGE !=  type) {
            loaded = true;
        } else {
...
//	                iconLoadListener.onIconLoadFinished(view);
    ...
    public void loadPhotoFromServer(ContentResolver resolver, String url) {
        ...
        if (url!=null) {
            KanboxIconDownloader task = new KanboxIconDownloader(this, url);
    ...
    private int saveIconIntoDb(String url, byte[] data) {
        ...
        int result = mResolver.update(DataStructures.CloudBoxColumns.CONTENT_URI, values,
                DataStructures.CloudBoxColumns.FILE_PATH_FIELD + "=?", new String[]{url});
        return result;
    }
