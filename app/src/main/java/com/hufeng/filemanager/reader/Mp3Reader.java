package com.hufeng.filemanager.reader;
...
public class Mp3Reader {
	...
	private static final String LOG_TAG = "Mp3Reader";
	...
	public Mp3Reader(File file)  {
		try {
			...
			if(ran==null || ran.length()<128)
				return;
			ran.seek(ran.length() - 128);
			ran.read(buffer);
			info = new SongInfo(buffer);
			if(LogUtil.IDBG) LogUtil.i(LOG_TAG,"name:" + info.getSongName() + " year:"
	...
	public static void main(String[] args) throws IOException {
		Mp3Reader read = new Mp3Reader(new File("/Users/feng/test.mp3"));
		byte[] buffer = new byte[128];

		read.ran.seek(read.ran.length() - 128);

		read.ran.read(buffer);
		SongInfo info = read.new SongInfo(buffer);
	...
	public class SongInfo {
		...
		public byte[] getBytes() {
			...
			System.arraycopy(temp, 0, data, 3, temp.length > 30 ? 30
					: temp.length);
			temp = artist.getBytes();
			System.arraycopy(temp, 0, data, 33, temp.length > 30 ? 30
					: temp.length);
			temp = album.getBytes();
			System.arraycopy(temp, 0, data, 63, temp.length > 30 ? 30
					: temp.length);
			temp = year.getBytes();
			System.arraycopy(temp, 0, data, 93, temp.length > 4 ? 4
					: temp.length);
			temp = comment.getBytes();
			System.arraycopy(temp, 0, data, 97, temp.length > 28 ? 28
					: temp.length);
			data[125] = r1;
			data[126] = r2;
			data[127] = r3;
